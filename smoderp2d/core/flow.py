...
class Mfda(object):
    ...
    def cell_runoff(self, i, j, sur=True):
        inflow_from_cells = \
            ...
            self.arr[i - 1][j + 1][22] + \
            self.inflows[i][j - 1][0] * self.arr[i][j - 1][22] + \
            self.inflows[i][j + 1][4] * self.arr[i][j + 1][22] + \
            self.inflows[i + 1][j - 1][7] * self.arr[i + 1][j - 1][22] + \
            self.inflows[i + 1][j][6] * self.arr[i + 1][j][22] + \
