## @package main_src.data_preparation Method to performe the preprocessing with arcpy package
...
def zapis (name, array_export, l_x,l_y,spix,vpix, NoDataValue,folder):
    ...
    raster = arcpy.NumPyArrayToRaster(array_export, ll_corner, spix, vpix, NoDataValue)
...
def prepare_data(args):
  ...
  def addfield(inpute, newfield, datatyp, default_value): #EDL
      ...
      arcpy.CalculateField_management(inpute, newfield, default_value, "PYTHON")
      return inpute
  ...
  zapis("fl_dir",mat_fd,x_coordinate,y_coordinate,spix,vpix,NoDataValue,temp)
      ...
      for i in range (rows):
        for j in range(cols):
            ...
            try:
              ...
              combinat.append(ccc)
              combinat.index(ccc)
              combinatIndex.append([combinat.index(ccc), ccc[0],ccc[1], 0])
              mat_inf_index [i][j] = combinat.index(ccc)
  ...
  if points and points != "#":
      for kyk in range( array_points.shape[0] - 1 ):
          if array_points[kyk][1] == i and array_points[kyk][2] == j:
              gp.AddMessage("Point FID = " + str( int( array_points[kyk][0] ) ) + " is at the edge of the raster. This point will not be included in results.")
          ...
          for j in range(cols):
              if mat_slope[i][j] != NoDataValue and mat_tau[i][j] != NoDataValue:
                  ...
                  if slope == 0.0:
                      ...
                      hcrit_v = np.power((v_crit/aa),exp) # h critical from v (10 je kuli jednotkam, bude jinak)
